...
The `oro.nifti::orthographic` function displays `nifti` objects in 3 different planes.  
...
High intensities can "dampen" the viewing of an image. 
...
We see that the white matter is represented here, but we would like to see areas of the brain that are not over this quantile to be shown as the image. 
...
The `ortho2` (and `orthograhic`) function is based on the `graphics::image` function in R, as well as many other functions we will discuss below.  When `graphics::image` sees an `NA` value, it does not plot anything there.  
...
Sometimes you would like to represent 2 images side by side, of the same dimensions and orientation of course.  The `double_ortho` function allows you to do this.  Let's get a coarse mask of the image using the `oMask` function:
...
We can view the original T1 alongside the estimated mask (useful for checking registration):
...
This allows the user to use a "generic" version of `image`, which `oro.nifti` adapted specifically for `nifti` objects.  You can see the help for this `nifti`-specific function in `?image.nifti`.

## Plotter

Let's **try** to plot an image of the $80^{th}$ slice of the T1 image:
```{r all_slices}
...
In `oro.nifti` version `0.7.4`, we added `slice`, which is the same as `image`, but `plot.type = "single"` as default:
...
There is no `overlay2` because we added `NA.y` in `oro.nifti` version `0.7.4`:
