...
author: "Kristin Linn Jean-Philippe Fortin"
date: "`r Sys.Date()`"
output:
...
t1s = lapply(t1_fnames["MPRAGE"], readnii)
tissues = lapply(t1_fnames["Tissue_Classes"], readnii)
```
...
plot(gmDensWB[[1]], xlim=c(gmMinXWB, gmMaxXWB), ylim=c(0, gmMaxYWB), xlab="Whole-brain Normalized Intensities", main="Gray Matter")
...
plot(wmDensWB[[1]], xlim=c(wmMinXWB, wmMaxXWB), ylim=c(0, wmMaxYWB), xlab="Whole-brain Normalized Intensities", main="White Matter")
...
plot(csfDensWB[[1]], xlim=c(csfMinXWB, csfMaxXWB), ylim=c(0, csfMaxYWB), xlab="Whole-brain Normalized Intensities", main="CSF")
...
```{r wsViz, warning=FALSE, message=FALSE}
gmDensWS = list()
for(i in 1:5){
  gmDensWS[[i]] = density(t1WSNorm[[i]][tissues[[i]]==3])
}
...
wmDensWS = list()
for(i in 1:5){
  wmDensWS[[i]] = density(t1WSNorm[[i]][tissues[[i]]==2])
}
