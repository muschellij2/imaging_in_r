...
- Subsetting a matrix - `[row,column]` format, if `row` or `column` missing then all values:
```{r matSub}
print(m)
print(m[,4])
print(m[2,])
...
Or directly pass in a vector of logicals:
```{r}
v[ v > 5 ]
```
...
We will read in the `training01_01_mprage.nii.gz` file, and assign it to an object called `t1`:
```{r}
library(neurobase)
...
- similarly, `dir.exists` for directories
