...
* `oro.nifti` - reading/writing NIfTI images
    - made the `nifti` object: like an array - but with header information
...
The `t1 > 400` creates a `nifti` object with all logical values:
```{r}
...
If you do `array(c(t1), dim = dim(t1))`, this will put things back "in order" into an array.
