...
- In this tutorial we will use ComBat [@fortin2018harmonization] to harmonize cortical thicknesses from the ADNI data.
...
- The Alzheimer's Disease Neuroimaging Initiative (ADNI) is a multi-million dollar study funded by public and private resources. 
...
- We estimated cortical thicknesses from a subset of initial subject visits (N=215)
    - Mix of healthy controls, MCI, and AD diagnoses at baseline
...
## ComBat Model
- ComBat [@johnson2007adjusting] has been shown to remove unwanted variation due to scanner while preserving biological associations [@fortin2018harmonization].
- ComBat assumes expected values of the imaging feature measurements can be modeled as a linear combination of the biological variables and the site effects with an error term that includes a multiplicative scanner-specific scaling factor:
  $$y_{i,j,v} = \alpha_v + X_{i,j}^{T}\beta_v + Z_{i,j}^{T}\theta_v + \delta_{i,j}\epsilon_{i,j,v}$$ 
    - $y_{i,j,v}$ is average cortical thickness in the $v^{th}$ ROI from subject $i$, scanner $j$
    - $X_{i,j}^{T}$ is a vector of fixed covariates for subject $i$ scanned on scanner $j$
    - $\alpha_v$, $\beta_v$ are the intercept and slope vector of covariates for the $v^{th}$ imaging measurement
...
- Substituting $\gamma_{i,v}$ for $Z_{i,j}^{T}\theta_v$ we have:
  $$y_{i,j,v} = \alpha_v + X_{i,j}^{T}\beta_v + \gamma_{i,v} + \delta_{i,j}\epsilon_{i,j,v}$$ 
- ComBat uses empirical Bayes estimation to improve quality of scanner-specific parameter estimates when sample sizes are small 
    - Let $\gamma_{i,v}^{*}$ and $\delta_{i,v}^{*}$ denote the EB estimates of $\gamma_{i,v}$ and $\delta_{i,v}$, respectively.
- Then, the ComBat-harmonized cortical thicknesses are defined as 
$$y_{i,j,v}^{ComBat} = \frac{y_{i,j,v} - \hat{\alpha}_v - X_{i,j}^{T}\hat{\beta}_v - \gamma_{i,v}^{*}}{\delta_{i,v}^{*}}  + \hat{\alpha}_v + X_{i,j}^{T}\hat{\beta}_v$$
